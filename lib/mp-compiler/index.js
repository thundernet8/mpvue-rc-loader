...
function compileWxml (compiled, html) {
  if (!emitHookAdded) {
    ...
    this._compilation.plugin('seal', () => {
      console.log('emitQueue', emitQueue);

      const roots = Object.keys(emitQueue).filter(k => emitQueue[k].isRoot).map(k => emitQueue[k]);
      const tpls = Object.keys(emitQueue).filter(k => !emitQueue[k].isRoot).reverse().map(k => emitQueue[k]).reduce((prev, curr) => {
        return prev + curr.code + '\r\n'
      }, '');
      roots.forEach(root => {
        this.emitFile(`${root.path}.${root.type}`, tpls+'\r\n'+root.code)
      })
