...
function getSlots() {
    const allImportCode = Object.keys(importCodeCache)
        .map(v => importCodeCache[v])
        .join('\n')
        .replace('<import src="slots" />', '');
    const allSlots = Object.keys(slotsCache)
        .map(v => slotsCache[v].code)
        .join('\n');
    return allImportCode + allSlots;
}

function getSlotsWithOutImports() {
    const allImportCode = Object.keys(importCodeCache)
        .map(v => importCodeCache[v])
        .filter(v => v && !v.startsWith('<import src=') && !v.startsWith('<import src="slots" />'))
        .join('\n')
        .replace('<import src="slots" />', '');
    const allSlots = Object.keys(slotsCache)
        .map(v => slotsCache[v].code)
