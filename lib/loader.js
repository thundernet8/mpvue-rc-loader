...
// for mp js
var { compileMP, compileMPScript, compileMPComponent } = require('./mp-compiler');
...
module.exports = function(content) {
    ...
    const isEntryComponent =
        Object.keys(this.options.entry)
            .map(k => this.options.entry[k])
            .indexOf(this.resourcePath) > -1;

    if (isEntryComponent) {
        // 说明是组件入口
        compileMPComponent.call(this, content, mpOptions);
        ...
        //   app.$mount();`
        // compileMP.call(this, newContent, mpOptions)
    }
    ...
    var defaultLoaders = {
        ...
        css: options.extractCSS ? getCSSExtractLoader() : styleLoaderPath + '!' + 'css-loader' + cssLoaderOptions,
    ...
    var needsHotReload = !isServer && !isProduction && (parts.script || parts.template);
    ...
    if (script) {
        ...
        if (options.esModule) {
            output += script.src ? getImportForImport('script', script) : getImport('script', script) + '\n';
        } else {
            output +=
                'var __vue_script__ = ' +
                (script.src ? getRequireForImport('script', script) : getRequire('script', script)) +
                '\n';
        }
        ...
        // __vue_script__.mpType = 'component';
        // new Vue(__vue_script__).$mount();`;
        //     }
    } else {
    ...
    if (template) {
        if (options.esModule) {
            output +=
                (template.src ? getImportForImport('template', template) : getImport('template', template)) + '\n';
        } else {
            output +=
                'var __vue_template__ = ' +
                (template.src ? getRequireForImport('template', template) : getRequire('template', template)) +
                '\n';
        }
    } else {
    ...
    function getRequireForImportString(type, impt, scoped) {
        return loaderUtils.stringifyRequest(loaderContext, '!!' + getLoaderString(type, impt, -1, scoped) + impt.src);
    }
    ...
    function getRawLoaderString(type, part, index, scoped) {
        ...
        var loader =
            options.extractCSS && type === 'styles' ? loaders[lang] || getCSSExtractLoader(lang) : loaders[lang];
    ...
    function getSelectorString(type, index) {
        return (
            selectorPath +
            '?type=' +
            (type === 'script' || type === 'template' || type === 'styles' ? type : 'customBlocks') +
            '&index=' +
            index +
            '!'
        );
    ...
    function getCSSExtractLoader(lang) {
        ...
        var langLoader = lang ? ensureBang(ensureLoader(lang)) : '';
